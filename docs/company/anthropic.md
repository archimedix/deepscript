# Anthropic

> AI safety company che sviluppa Claude, fondata da ex-OpenAI per creare AI affidabile e sicura

## Executive Summary

| Metrica | Valore |
|---------|--------|
| **Fondazione** | 2021 |
| **Sede** | San Francisco, USA |
| **Tipo** | Company (AI) |
| **Leadership** | Dario Amodei (CEO), Daniela Amodei (President) |
| **Dipendenti** | ~1,500+ (2025) |
| **Valutazione** | $183B (Sep 2025) |
| **Revenue** | $5B+ run rate (Aug 2025), ~$7B ARR projected EOY |
| **Profitability** | Attesa 2028 |
| **Status** | active |

---

## Fondazione e Storia

| Dato | Dettaglio |
|------|-----------|
| **Fondazione** | 2021 |
| **Fondatori** | Dario Amodei, Daniela Amodei, Tom Brown, Chris Olah, Sam McCandlish, Jared Kaplan, Jack Clark, Ben Mann |
| **Contesto** | Scissione da OpenAI per divergenze sulla direzione e sicurezza dell'AI |
| **Obiettivo dichiarato** | Sviluppare sistemi AI sicuri, benefici e comprensibili |

### Timeline Storica

| Anno | Evento |
|------|--------|
| **2021** | Fondazione da 8 ex-OpenAI |
| **2023 Q1** | Lancio Claude, primo modello pubblico |
| **2023 May** | Serie C $450M (Spark Capital, Google) - valutazione $4.1B |
| **2023 Sep** | Amazon annuncia investimento fino a $4B |
| **2023 Oct** | Google investe $2B |
| **2024 Feb** | Serie D $750M (Menlo Ventures) |
| **2024 May** | Mike Krieger (co-founder Instagram) entra come CPO |
| **2024 Nov** | Amazon completa $8B totali, AWS diventa primary cloud partner |
| **2025 Jan** | Google aggiunge $1B (totale ~$3B) |
| **2025 Mar** | Serie E $3.5B (Lightspeed) - valutazione $61.5B |
| **2025 Aug** | Revenue $5B+ run rate (5x in 8 mesi) |
| **2025 Sep** | Serie F $13B (ICONIQ, Fidelity, Lightspeed) - valutazione $183B |
| **2025 Dec** | Enterprise market share 40% (Menlo report) |
| **2025** | Contratti defense con DoD e intelligence agencies |
| **2025 Oct** | Rahul Patil (ex-CTO Stripe) entra come CTO; Sam McCandlish diventa Chief Architect |
| **2025 Dec** | Lancio Anthropic Labs (Mike Krieger + Ben Mann), Ami Vora Head of Product |

---

## Struttura e Governance

### Leadership Attuale

| Ruolo | Nome | Dal |
|-------|------|-----|
| CEO | [Dario Amodei](../persons/dario-amodei.md) | 2021 |
| President | [Daniela Amodei](../persons/daniela-amodei.md) | 2021 |
| Chief Science Officer | [Jared Kaplan](../persons/jared-kaplan.md) | 2021 |
| Chief Architect | [Sam McCandlish](../persons/sam-mccandlish.md) | 2025 (ex tech lead) |
| Head Mechanistic Interpretability | [Chris Olah](../persons/chris-olah.md) | 2021 |
| Head Core Resources | [Tom Brown](../persons/tom-brown.md) | 2021 |
| Head of Policy | [Jack Clark](../persons/jack-clark.md) | 2021 |
| Chief Technology Officer | [Rahul Patil](../persons/rahul-patil.md) | 2025 |
| Head of Product | [Ami Vora](../persons/ami-vora.md) | 2025 |
| Labs | [Mike Krieger](../persons/mike-krieger.md), [Ben Mann](../persons/ben-mann.md) | 2025 |

### Riorganizzazione Leadership 2025

| Cambio | Dettaglio |
|--------|-----------|
| **Rahul Patil** | Ex-CTO Stripe, entra come CTO (Oct 2025) |
| **Sam McCandlish** | Da tech lead a Chief Architect |
| **Ami Vora** | Ex-Meta/WhatsApp, entra come Head of Product (Dec 2025) |
| **Mike Krieger** | Da CPO a Labs (Dec 2025) |
| **Reporting** | CTO e Chief Architect riportano a Daniela Amodei |

### Anthropic Labs

Team dedicato a prodotti sperimentali alla frontiera delle capacita' di Claude:
- **Lead**: Mike Krieger (ex-CPO, co-founder Instagram) + Ben Mann
- **Focus**: Tinkering e sperimentazione, test con early users
- **Obiettivo**: Bilanciare scoperta di nuovi prodotti con scaling delle esperienze Claude esistenti

### Caratteristiche Governance

- Public Benefit Corporation (PBC)
- Focus su AI safety e Constitutional AI
- Long-Term Benefit Trust per proteggere la mission
- Responsible Scaling Policy: framework interno per valutare rischi prima di rilascio modelli
- Jared Kaplan: Responsible Scaling Officer (ruolo formale per enforcement safety)

---

## Prodotti

| Prodotto | Descrizione |
|----------|-------------|
| **Claude** | Large Language Model (Claude 3.5, Claude Opus 4.5) |
| **Claude API** | API per sviluppatori |
| **Claude Code** | CLI per coding assistito |
| **Claude for Enterprise** | Soluzioni enterprise |
| **Constitutional AI** | Metodologia di training per AI sicura |
| **Mechanistic Interpretability** | Ricerca su comprensione interna dei modelli (goal 2027) |

---

## Investitori Chiave

### Strategic Investors

| Investitore | Investimento | Note |
|-------------|--------------|------|
| [Amazon](amazon.md) | $8B | AWS primary cloud partner, ~10-15% stake |
| [Google](../company/google.md) | ~$3B | ~10% stake |
| [Salesforce](../company/salesforce.md) | - | Via Salesforce Ventures |

### Venture Capital

| Investitore | Round | Note |
|-------------|-------|------|
| [Spark Capital](../vc/spark-capital.md) | Series C lead | $450M (May 2023) |
| [Menlo Ventures](../vc/menlo-ventures.md) | Series D lead | $750M (Feb 2024), Anthology Fund partner |
| [Lightspeed Venture Partners](../vc/lightspeed-venture-partners.md) | Series E lead | $3.5B (Mar 2025), $1B invested |
| [ICONIQ Capital](../asset-manager/iconiq-capital.md) | Series F lead | $13B (Sep 2025) |
| [Fidelity](../asset-manager/fidelity.md) | Series F co-lead | Also Series E participant |
| Qatar Investment Authority | Series F | Sovereign wealth fund |

### Anthology Fund

Partnership [Menlo Ventures](../vc/menlo-ventures.md) + Anthropic (Jul 2024):
- **Size**: $100M
- **Focus**: Startup che innovano con tecnologia Anthropic
- **Prima coorte**: 18 startup (Dec 2024)

---

## Market Position (2025)

| Metrica | Valore | Fonte |
|---------|--------|-------|
| **Enterprise LLM API market share** | 40% | Menlo Ventures Report Dec 2025 |
| **vs 2023** | 12% → 40% (3x) | - |
| **OpenAI share** | 27% | - |
| **Google share** | 21% | - |
| **Revenue growth** | $1B → $5B+ in 8 mesi (2025) | - |
| **Ranking startup** | #4 most valuable globally | Sep 2025 |

---

## Connessioni PowerLink

### Origine da OpenAI

Gli 8 fondatori lasciarono [OpenAI](openai.md) nel 2021 per divergenze sulla direzione strategica e l'approccio alla sicurezza:

| Persona | Ruolo OpenAI | Pre-OpenAI | Ruolo Anthropic |
|---------|--------------|------------|-----------------|
| Dario Amodei | VP Research (GPT-2/3) | Google Brain, Baidu | CEO |
| Daniela Amodei | VP Safety & Policy | Stripe | President |
| Tom Brown | Lead Engineer GPT-3 | Google Brain | Head Core Resources |
| Chris Olah | Lead Interpretability | Google Brain | Head Interpretability |
| Sam McCandlish | Research Lead | Boston University (Postdoc) | Chief Architect (2025, ex tech lead) |
| Jared Kaplan | Researcher | Johns Hopkins (Professor) | Chief Science Officer |
| Jack Clark | Policy Director | Bloomberg (Tech Reporter) | Head of Policy |
| Ben Mann | Technical Staff (GPT-3) | Google, MIRI | Labs (ex Tech Lead Product Eng.) |

### Formazione Founders

| Persona | Universita | Titolo |
|---------|------------|--------|
| Dario Amodei | Stanford, Princeton | BS Physics, PhD Biophysics (Hertz Fellow) |
| Daniela Amodei | UC Santa Cruz | BA English/Music (Summa Cum Laude) |
| Jared Kaplan | Stanford, Harvard | BS Physics & Math, PhD Physics (Hertz Fellow) |
| Sam McCandlish | Brandeis, Stanford | BS/MS Physics, PhD Theoretical Physics (DOE Fellow) |
| Tom Brown | MIT | Undergraduate (self-taught ML) |
| Chris Olah | Thiel Fellowship | No degree (dropout, $100K grant 2012) |
| Jack Clark | Univ. of East Anglia | BA English Literature (humanities background) |
| Ben Mann | Columbia | BS Computer Science |

**Note**: 2 Hertz Fellows (Dario, Jared), 1 DOE Fellow (Sam), 1 Thiel Fellow (Chris). Solo 3 degli 8 founders non hanno titolo universitario avanzato (Tom, Chris, Jack). Jack Clark e' l'unico con background non-STEM.

### Contributi Tecnici dei Founders

| Persona | Contributo chiave |
|---------|-------------------|
| Dario Amodei | Co-inventore RLHF, GPT-2/3 |
| Tom Brown | Lead engineer GPT-3, 113K+ citazioni |
| Chris Olah | Pioniere mechanistic interpretability, 103K+ citazioni |
| Jared Kaplan | Scaling Laws (lead author), Constitutional AI, 125K+ citazioni |
| Sam McCandlish | Scaling Laws (co-author), architettura Claude |
| Jack Clark | AI Index Stanford (co-founder), AI policy framework |
| Ben Mann | GPT-3 co-architetto, demo Microsoft, Claude Code tech lead |

### Riconoscimenti Founders

| Anno | Founder | Riconoscimento |
|------|---------|----------------|
| **2023** | Dario Amodei | TIME 100 Most Influential in AI |
| **2023** | Daniela Amodei | TIME 100 Most Influential in AI |
| **2024** | Chris Olah | TIME 100 Most Influential in AI |
| **2025** | Dario Amodei | TIME 100 Most Influential People, "Architects of AI" |
| **2025** | Daniela Amodei | Fortune Most Powerful Women (#1) |
| **2025** | Jared Kaplan | TIME 100 Most Influential in AI |

### Competizione AI

| Azienda | Relazione |
|---------|-----------|
| [OpenAI](openai.md) | Competitor principale, origine dei fondatori |
| [Google](../company/google.md) | Investitore e competitor (Gemini) |
| [Meta](meta.md) | Competitor (LLaMA) |
| Microsoft | Investitore recente |
| Nvidia | Investitore recente |

---

## Critiche

| Tema | Dettaglio |
|------|-----------|
| Concentrazione potere | Poche aziende controllano AI frontier |
| Dual-use | Potenziale uso militare/sorveglianza |
| Race dynamics | Pressione competitiva vs safety |
| Defense contracts | Contratti DoD 2025, critiche da parte della community |
| Regulatory capture | Accuse da David Sacks (Trump AI czar) di "fear-mongering" |

---

## Fonti

### Ufficiali
- [Anthropic.com](https://www.anthropic.com)
- [Anthropic Blog](https://www.anthropic.com/news)
- [Rahul Patil joins Anthropic](https://www.anthropic.com/news/rahul-patil-joins-anthropic)
- [Introducing Anthropic Labs](https://www.anthropic.com/news/introducing-anthropic-labs)

### Wikipedia
- [Anthropic - Wikipedia](https://en.wikipedia.org/wiki/Anthropic)
- [Dario Amodei - Wikipedia](https://en.wikipedia.org/wiki/Dario_Amodei)
- [Daniela Amodei - Wikipedia](https://en.wikipedia.org/wiki/Daniela_Amodei)

### Analisi
- [Contrary Research - Anthropic](https://research.contrary.com/company/anthropic)
- [CNBC - Anthropic valuation](https://www.cnbc.com/2025/03/03/amazon-backed-ai-firm-anthropic-valued-at-61point5-billion-after-latest-round.html)
- [Menlo Ventures - 2025 State of Generative AI Report](https://www.globenewswire.com/news-release/2025/12/09/3202258/0/en/Menlo-Ventures-2025-State-of-Generative-AI-Report.html)

---

*Ultimo aggiornamento: Febbraio 2026*
