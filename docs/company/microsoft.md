# Microsoft

> Da garage a $3 trilioni: il gigante del software che ora domina l'era AI

## Executive Summary

| Metrica | Valore |
|---------|--------|
| **Fondazione** | 1975 |
| **Sede** | Redmond, Washington |
| **Tipo** | Company (Tech) |
| **Leadership** | Satya Nadella (CEO & Chairman) |
| **Dipendenti** | ~220,000 |
| **Market Cap** | ~$3T (2025) |
| **Status** | active |

---

## Glossario

| Termine | Definizione |
|---------|-------------|
| Azure | Piattaforma cloud Microsoft |
| Copilot | Assistente AI integrato nei prodotti Microsoft |
| GAFAM | Google, Apple, Facebook, Amazon, Microsoft |

---

## Fondazione e Storia

| Dato | Dettaglio |
|------|-----------|
| **Fondazione** | 4 aprile 1975 |
| **Fondatori** | Bill Gates, Paul Allen |
| **Contesto** | Rivoluzione personal computer |
| **Prima sede** | Albuquerque, New Mexico |
| **Breakthrough** | Deal IBM per DOS (1980) |

### Timeline Storica

| Anno | Evento |
|------|--------|
| **1975** | Fondazione da Gates e Allen |
| **1980** | Deal IBM per MS-DOS |
| **1985** | Lancio Windows 1.0 |
| **1986** | IPO |
| **1995** | Windows 95, Internet Explorer |
| **2000** | Gates lascia ruolo CEO a Ballmer |
| **2001** | Lancio Xbox |
| **2011** | Acquisizione Skype ($8.5B) |
| **2014** | Satya Nadella CEO |
| **2016** | Acquisizione LinkedIn ($26B) |
| **2019** | Primo investimento OpenAI ($1B) |
| **2021** | Nadella diventa anche Chairman |
| **2022** | Acquisizione Activision Blizzard ($69B) |
| **2023** | Integrazione ChatGPT in Bing |
| **2024** | Stake 27% in OpenAI ($135B valore) |

---

## Struttura e Governance

### Leadership Attuale

| Ruolo | Nome | Dal |
|-------|------|-----|
| CEO & Chairman | Satya Nadella | 2014 (CEO), 2021 (Chairman) |
| CFO | Amy Hood | 2013 |
| President | Brad Smith | 2015 |
| Lead Independent Director | Sandra Peterson | 2023 |

### Board of Directors

| Nome | Background |
|------|------------|
| Satya Nadella | CEO & Chairman |
| Reid Hoffman | Co-founder LinkedIn, Greylock |
| Penny Pritzker | Ex Sec. Commerce USA, Pritzker family |
| Hugh Johnston | CFO PepsiCo |
| Sandra Peterson | Ex J&J |

---

## Azionariato

### Principali Azionisti Istituzionali

| Investitore | Quota |
|-------------|-------|
| [Vanguard](../asset-manager/vanguard.md) | ~8% |
| [BlackRock](../asset-manager/blackrock.md) | ~7.2% |
| [State Street](../asset-manager/state-street.md) | ~3.5% |
| [Norges Bank (GPFG)](../swf/norges-bank-gpfg.md) | ~1.1% |

### Fondatori/Insider

| Nome | Quota | Note |
|------|-------|------|
| Bill Gates | <1% | Venduto gran parte nel tempo |
| Steve Ballmer | ~4% | Maggior azionista individuale |

---

## Investimenti Strategici

### OpenAI Partnership

| Aspetto | Dettaglio |
|---------|-----------|
| **Primo investimento** | $1B (2019) |
| **Totale investito** | $13B+ |
| **Quota attuale** | ~27% della PBC |
| **Valore stake** | ~$135B (2025) |
| **Accordo Azure** | $250B in servizi cloud |

### Altre Acquisizioni Rilevanti

| Anno | Azienda | Valore |
|------|---------|--------|
| 2016 | LinkedIn | $26B |
| 2018 | GitHub | $7.5B |
| 2022 | Activision Blizzard | $69B |

---

## Connessioni PowerLink

### Figure Chiave

| Persona | Ruolo |
|---------|-------|
| [Bill Gates](../persons/bill-gates.md) | Founder, ex-CEO |
| [Satya Nadella](../persons/satya-nadella.md) | CEO & Chairman |
| [Steve Ballmer](../persons/steve-ballmer.md) | Ex-CEO 2000-2014 |
| [Reid Hoffman](../persons/reid-hoffman.md) | Board member |

### Organizzazioni Correlate

| Org | Relazione |
|-----|-----------|
| [OpenAI](openai.md) | 27% stake, partner strategico |
| [WEF](../forum/wef.md) | Strategic Partner |
| [BlackRock](../asset-manager/blackrock.md) | Top shareholder |
| [Vanguard](../asset-manager/vanguard.md) | Top shareholder |

### Connessioni Forum

| Forum | Connessione |
|-------|-------------|
| Bilderberg | Gates, Nadella partecipanti |
| WEF Davos | Partner strategico, Nadella speaker |

---

## AI Strategy

### Posizionamento

Microsoft ha scommesso sull'AI come next platform shift:

1. **OpenAI partnership**: accesso esclusivo a GPT, investimento $13B+
2. **Copilot**: AI integrata in Office, Windows, GitHub
3. **Azure AI**: infrastruttura cloud per training/inference
4. **Bing Chat**: sfida a Google Search

### Infrastruttura

- $80B in data center AI entro 2025
- Oltre 50% investimenti in USA
- Partnership NVIDIA per chip AI

---

## Critiche

| Tema | Dettaglio |
|------|-----------|
| **Antitrust storico** | Causa DOJ anni '90 (browser bundling) |
| **Monopolio cloud** | Concentrazione Azure + OpenAI |
| **Privacy** | Telemetria Windows, LinkedIn data |
| **Gaming** | Acquisizione Activision contestata |

---

## Fonti

### Ufficiali
- [Microsoft](https://www.microsoft.com)
- [Microsoft Leadership](https://news.microsoft.com/source/leadership/)

### Wikipedia
- [Microsoft - Wikipedia](https://en.wikipedia.org/wiki/Microsoft)
- [Satya Nadella - Wikipedia](https://en.wikipedia.org/wiki/Satya_Nadella)

### Analisi
- [The Conversation - Microsoft 50 years](https://theconversation.com/from-ibm-to-openai-50-years-of-winning-and-failed-strategies-at-microsoft-253576)

---

*Ultimo aggiornamento: Dicembre 2025*
